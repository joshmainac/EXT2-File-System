...
int my_symlink()
{
  ...
  // strcat(child, " -> ");
  // // may want to tok old_file
  // strcat(child, old_file);
  ///////////////////////////my_ksymlink(pmip, child);
  ...
  mip->dirty = 1;
  printf("my_symlink-8 %s\n", (char *)mip->INODE.i_block, old_file);
  ...
  for (int i = 0; i < 12; i++)
  {
    printf("my_symlink-8A %d\n", pip->i_block[i]);
  }

  // eneter name has a problem
